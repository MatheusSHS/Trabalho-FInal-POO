...
public class GerenciadorMapa {
    ...
    public GerenciadorMapa(GeoPosition centro, FonteImagens fonte) {
        ...
        pontosPainter.setRenderer((Graphics2D g, JXMapViewer viewer, MyWaypoint wp) -> {
        ...
        Painter<JXMapViewer> linhasPainter = (Graphics2D g, JXMapViewer map, int w, int h) -> {
            linhas.forEach((tr) -> {
...
g.drawPolyline(x, y, x.length);
        ...
        Painter<JXMapViewer> guiPainter = (Graphics2D g, JXMapViewer map, int w, int h) -> {
            if(selCentro == null)
                return;
